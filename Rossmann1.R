...
combotest.combined$SchoolHoliday <- as.factor(combotest.combined$SchoolHoliday)
...
table(combotest.combined$SchoolHoliday)
levels(combotest.combined$SchoolHoliday) <- c("NotAffectBySH", "AffectedBySchoolH")
...
#Single store '622' responsible for all NA's in Open variable. 
#should be 0 as per Kaggle post by company executive
test[is.na(test$Open),]
test[is.na(test$Open), ]$Open <- 0
...
#any days of Sales greater than 0 and stores closed?
summary(train$Sales[!train$Open])
#Negative
#any days of Sales greater than 0 and No customers?
summary(train$Sales[train$Customers == 0])
#Negative
